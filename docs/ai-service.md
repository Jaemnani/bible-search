...
aibtown.com 의 `/ai-services` 그리드에 새 GitHub 프로젝트를 노출시키기 위한 절차. 카드 / 상세 페이지가 비주얼이 빈약하지 않게 보이도록 (A) 대상 레포 측 README/메타데이터 정비와 (B) knowai-space 측 DB 등록 두 단계로 나누어 안내한다.
...
```bash
gh label create roadmap --color FBCA04 --description "Workshop roadmap on aibtown.com"
...
1. `https://www.aibtown.com/admin` 접근 (계정의 `profiles.role` 이 `admin` 또는 `moderator` 여야 함).
...
```bash
curl -X POST https://www.aibtown.com/api/admin/workshop \
...
응답 `201 { item: { ..., slug } }`. URL 은 `https://www.aibtown.com/ko/ai-services/<slug>`.
...
등록 후 URL: `https://www.aibtown.com/ko/ai-services/bible-search`.
